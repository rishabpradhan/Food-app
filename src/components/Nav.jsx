...
export default function Nav() {
  ...
  return (
    <>
      <nav
        className=" bg-white fixed  top-0 left-0 w-full z-40 h-10
            ...
            <ul className="sm:flex gap-10 justify-end">
              {Navlinks.map(({ id, link, name }) => (
                <li key={id}>
                  <Link
                    to={link}
                    className="inline-block font-bold hover:text-purple-500  text-xl"
                  >
                    {name}{" "}
                  </Link>
                </li>
      ...
      <div className="  mt-[4rem] text-center overflow-x-hidden ">
        <span className="uppercase font-inria text-xl">
          elegant recipes made for{" "}
        </span>
        <span className=" p-3 font-italianno text-purple-700 text-3xl ">
          {" "}
          everyday life
        </span>
      </div>
